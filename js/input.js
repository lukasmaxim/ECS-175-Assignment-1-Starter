...
class Input
{
    ...
    constructor( )
    {
        ...
        window.onkeydown = ( e ) => { this.down_keys[ e.key ] = true; this.pressed_keys[ e.key ] = true; this.last_key_pressed = e.key}
        window.onmousedown = ( e ) => { this.mouse_down[ e.button ] = true; this.mouse_clicked[ e.button ] = true}
        window.onmouseup = ( e ) => this.mouse_down[ e.button ] = false
        window.onmousemove = ( e ) =>
        {
            const pos = getRelativeMousePosition(e, e.target)
    ...
    isInBounds(canvas)
    {
        return this.mousex >= 0 && this.mousex <= canvas.width
    }
